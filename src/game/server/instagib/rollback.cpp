...
CCharacter *CRollback::IntersectCharacterOnTick(vec2 Pos0, vec2 Pos1, float Radius, vec2 &NewPos, const CCharacter *pNotThis, int CollideWith, const CCharacter *pThisOnly, int Tick)
{
	...
	for(; pChar; pChar = (CCharacter *)pChar->TypeNext())
	{
		...
		vec2 charpos;
		if(pChar->m_Positions[tick].m_Valid)
			charpos = pChar->m_Positions[tick].m_Position;
		else
			charpos = pChar->m_Pos;
...
int CRollback::FindCharactersOnTick(vec2 Pos, float Radius, CEntity **ppEnts, int Max, int Tick)
{
	...
	{
		if(pChr->m_Positions[tick].m_Valid ? (distance(pChr->m_Positions[tick].m_Position, Pos) < Radius + pChr->GetProximityRadius()) : (distance(pChr->m_Pos, Pos) < Radius + pChr->GetProximityRadius()))
		{
			if(ppEnts)
				ppEnts[Num] = pChr;
			Num++;
...
void CRollback::CreateExplosionOnTick(vec2 Pos, int Owner, int Weapon, bool NoDamage, int ActivatedTeam, int Tick, CClientMask Mask, CClientMask SprayMask)
{
	...
	for(int i = 0; i < Num; i++)
	{
		...
		vec2 Diff;
		if(!pChr->m_Positions[tick].m_Valid)
			Diff = pChr->m_Pos - Pos;
		else
			Diff = pChr->m_Positions[tick].m_Position - Pos;
		vec2 ForceDir(0, 1);
		float l = length(Diff);
		if(l)
